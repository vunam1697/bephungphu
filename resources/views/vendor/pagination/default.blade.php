...
@if ($paginator->lastPage() > 1)
    @if ($paginator->currentPage() > 1)
        <li class="{{ ($paginator->currentPage() == 1) ? ' disabled' : '' }} list-inline-item">
            <a href="{{ $paginator->url( $paginator->currentPage() - 1 ) }}" rel="First"><i class="fa fa-angle-left"></i></a>
        </li>
    @endif
    ...
    @if ($paginator->currentPage() < $paginator->lastPage())
            ...
            <a href="{{ $paginator->url( $paginator->currentPage() + 1 ) }}" rel="next"><i class="fa fa-angle-right"></i></a>
        </li>
    @endif
